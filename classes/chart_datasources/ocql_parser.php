...
class OCChartOcqlQueryParser
{
    ...
    public function __construct($stringParams)
    {
    ...
    private function parseRequest()
    {
        ...
        eZDebug::writeDebug( $this->rowField, 'rowField');
        ...
        foreach ($columnFields as $field) {
            ...
            if (!in_array($columnField['dataType'],
                array(eZIntegerType::DATA_TYPE_STRING, eZStringType::DATA_TYPE_STRING, eZSelectionType::DATA_TYPE_STRING, eZFloatType::DATA_TYPE_STRING))
            ) {
        ...
        eZDebug::writeDebug( $this->columnFields, 'columnFields');

        $this->options = isset( $parts[3] ) ? $this->explode(',', $parts[3]) : array();

        eZDebug::writeDebug( $this->options, 'options');
